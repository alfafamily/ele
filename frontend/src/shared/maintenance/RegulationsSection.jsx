...
import { ActionMenu, Badge, Button, ConfirmModal, Icon, InlineCalendar, Modal } from '../ui'
import { Tooltip } from '../Tooltip.jsx'
import { regulationPeriodLabel } from '../../features/types/regulationPeriodLabel.js'
import { RegulationFormModal } from '../../features/equipment/RegulationFormModal.jsx'
...
// B13+/B22. Сворачиваемый раздел «Регламенты ТО» на карточке объекта
// (оборудование/транспорт — зеркальные блоки). Показывает регламенты типа +
// индивидуальные с их планом; для управляющих ТО — действия: задать дату первого
// ТО, отменить/вернуть регламент, добавить/изменить/архивировать индивидуальный.
//
// props:
//   entity — объект { id, is_written_off };
//   regulations, canManage, onChanged — данные и колбэк перезагрузки;
//   entityGenitive — «оборудования»/«транспорта» для текстов;
//   planStatusIcon(status) — иконка статуса плана раздела;
//   api — { archive, create, restore, update, setCancelled, setDate } (все принимают objId первым аргументом).
export function RegulationsSection({ entity, regulations, canManage, onChanged, entityGenitive, planStatusIcon, api }) {
  ...
  const writtenOff = entity.is_written_off
  ...
  const saveDate = async () => {
    setBusy(true)
    try {
      await api.setDate(objId, dateTarget.id, dateValue)
      setDateTarget(null)
      reload()
    } finally {
      setBusy(false)
  ...
  const rowActions = (reg) => {
    ...
    if (reg.scope === 'individual') {
      ...
      items.push({ label: `Отменить для ${entityGenitive}`, danger: true, onClick: () => askCancel(reg, false) })
    }
  ...
  const restore = async (reg) => {
    if (reg.scope === 'individual' && reg.is_archived) {
      await api.restore(objId, reg.id)
    } else {
      await api.setCancelled(objId, reg.id, false)
    }
    reload()
  }
  ...
  const askCancel = (reg, isIndividualArchive) => {
    setConfirm({
      title: isIndividualArchive ? 'Отменить регламент?' : `Отменить регламент для ${entityGenitive}?`,
      message: isIndividualArchive
        ...
        : `Регламент «${reg.name}» перестанет контролироваться для этого ${entityGenitive}, плановая дата обнулится. Вернуть можно позже.`,
      confirmLabel: 'Отменить',
      onConfirm: async () => {
        if (isIndividualArchive) {
          await api.archive(objId, reg.id)
        } else {
          await api.setCancelled(objId, reg.id, true)
        }
        setConfirm(null)
        reload()
      },
          ...
          <div style={{ marginTop: 12 }}>
            ...
            {count === 0 ? (
              <div style={{ fontSize: 13.5, color: 'var(--color-text-muted)', marginTop: 12 }}>
                Для этого {entityGenitive} нет регламентов ТО.
              </div>
            ) : (
      ...
      {regModal ? (
        <RegulationFormModal
          ...
          onSave={async (payload) => {
            if (regModal === 'new') {
              await api.create(objId, payload)
            } else {
              await api.update(objId, regModal.id, payload)
            }
            setRegModal(null)
            reload()
          }}
