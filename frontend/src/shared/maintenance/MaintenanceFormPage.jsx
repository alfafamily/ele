...
// B13+/B22. Общий экран проведения ТО (оборудование и транспорт): сначала выбор
// регламента (первый пункт «Внеплановое ТО»), затем позиции (строки из регламента —
// read-only, можно отменить с причиной; свои строки — редактируемые), комментарий
// и дата следующего ТО. Приложение задаёт API, тексты, маршрут возврата и —
// опционально — компонент дополнительных полей (у транспорта это пробег/моточасы,
// который влияет на валидность формы и на payload через onChange).
//
// props:
//   getEntity(id), getRegulations(id), performMaintenance(id, payload) — API;
//   planStatusIcon(status) — иконка статуса плана раздела;
//   notFoundTitle, backLabel, backRoute — экран ошибки загрузки;
//   ExtraFields — необязательный компонент доп. полей: <ExtraFields entity onChange={fn} />,
//     где onChange({ valid, payload }) сообщает наверх валидность и часть payload.
export function MaintenanceFormPage({
  ...
  // Доп. поля раздела (пробег транспорта): { valid, payload }.
  const [extra, setExtra] = useState({ valid: true, payload: {} })
  ...
  const load = useCallback(() => {
    ...
    getEntity(id)
      .then((data) => {
        setEntity(data)
        return getRegulations(id)
      })
      .then(setRegulations)
      .catch(() => setLoadError(true))
  }, [id, getEntity, getRegulations])
  useEffect(load, [load])
  ...
  const canSubmit = !!chosen && activeItems.length > 0 && !cancelReasonMissing && dateOk && extra.valid
  ...
  if (loadError) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: 60, textAlign: 'center' }}>
        <div style={{ fontSize: 16, fontWeight: 600 }}>{notFoundTitle}</div>
        <Button variant="secondary" onClick={() => navigate(backRoute)}>{backLabel}</Button>
      </div>
    )
  }
  if (!entity || regulations === null) {
    return (
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ fontSize: 13.5, color: 'var(--color-text-placeholder)' }}>
          {entity.type_and_model} · {entity.inventory_number}
        </div>
      </div>
    </div>
  )
        ...
        {!chosen ? (
          ...
          <>
            <Card>
              ...
              {ExtraFields ? <ExtraFields entity={entity} onChange={setExtra} /> : null}
