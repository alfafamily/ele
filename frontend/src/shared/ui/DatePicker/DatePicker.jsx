...
// выбранный день — в чёрном круге.
export function DatePicker({ label, value, onChange, minDate, placeholder = 'Выберите дату', id }) {
  ...
  return (
    <div className="ele-datepicker" ref={ref}>
      {label ? (
        <label className="ele-datepicker__label" htmlFor={id}>
          {label}
        </label>
      ) : null}
      <button
        type="button"
        id={id}
        className={'ele-datepicker__trigger' + (open ? ' ele-datepicker__trigger--open' : '')}
        onClick={() => (open ? setOpen(false) : openCalendar())}
        aria-haspopup="dialog"
        aria-expanded={open}
      >
        <span className={selected ? 'ele-datepicker__value' : 'ele-datepicker__placeholder'}>
