...
// children(draft, setDraft) может вернуть JSX (одна колонка) ИЛИ объект
// { main, aside } — тогда на десктопе модалка шире и делится на 2 колонки:
// main слева, aside (блок «Размещение») справа. На мобильном — одна колонка.
export function FilterModal({ value, count = 0, onApply, onClear, isDraftActive, title = 'Фильтры', children }) {
  ...
  return (
    <div className="ele-filter-btn">
      ...
      {open ? (() => {
        const rendered = typeof children === 'function' ? children(draft, setDraft) : children
        // { main, aside } (plain-объект, не React-элемент) → две колонки.
        const split = rendered && typeof rendered === 'object' && !rendered.$$typeof && !Array.isArray(rendered) && ('main' in rendered || 'aside' in rendered)
        const mainNode = split ? rendered.main : rendered
        const asideNode = split ? rendered.aside : null
        return (
        <Modal open onClose={close} title={title} className={asideNode ? 'ele-modal--filter-wide' : undefined}>
          <div className={'ele-filter-modal__body' + (asideNode ? ' ele-filter-modal__body--cols' : '')}>
            <div className="ele-filter-modal__col">{mainNode}</div>
            {asideNode ? <div className="ele-filter-modal__col">{asideNode}</div> : null}
          </div>
