...
// B22. Проведение ТО транспорта — как у Оборудования, но дополнительно
// запрашивается текущий пробег/моточасы (необязательно; фиксируется в истории,
// на расчёт регламентов не влияет).
export function MaintenanceFormPage() {
  ...
  const load = useCallback(() => {
    ...
    getTransport(id)
      .then((data) => {
        setTransport(data)
        return getTransportRegulations(id)
      })
      .then(setRegulations)
  ...
  if (loadError) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12, padding: 60, textAlign: 'center' }}>
        ...
        <Button variant="secondary" onClick={() => navigate('/transport')}>К списку транспорта</Button>
      </div>
    )
  }
  if (!transport || regulations === null) {
    return (
    ...
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
      ...
      <div style={{ minWidth: 0 }}>
        ...
        <div style={{ fontSize: 13.5, color: 'var(--color-text-placeholder)' }}>
          {transport.type_and_model} · {transport.inventory_number}
        </div>
      </div>
    </div>
  )
