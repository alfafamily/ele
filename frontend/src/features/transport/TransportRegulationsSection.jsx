...
// B22. Сворачиваемый раздел «Регламенты ТО» на карточке транспорта (зеркало
...
export function TransportRegulationsSection({ transport, regulations, canManage, onChanged }) {
  ...
  const objId = transport.id
  const writtenOff = transport.is_written_off
  ...
  const saveDate = async () => {
    setBusy(true)
    try {
      await setRegulationDate(objId, dateTarget.id, dateValue)
      setDateTarget(null)
      reload()
    } finally {
      setBusy(false)
  ...
  const rowActions = (reg) => {
    ...
    if (reg.scope === 'individual') {
      ...
      items.push({ label: 'Отменить для транспорта', danger: true, onClick: () => askCancel(reg, false) })
    }
  ...
  const restore = async (reg) => {
    if (reg.scope === 'individual' && reg.is_archived) {
      await restoreTransportRegulation(objId, reg.id)
    } else {
      await setRegulationCancelled(objId, reg.id, false)
    }
    reload()
  }
  ...
  const askCancel = (reg, isIndividualArchive) => {
    setConfirm({
      title: isIndividualArchive ? 'Отменить регламент?' : 'Отменить регламент для транспорта?',
      message: isIndividualArchive
        ...
        : `Регламент «${reg.name}» перестанет контролироваться для этого транспорта, плановая дата обнулится. Вернуть можно позже.`,
      confirmLabel: 'Отменить',
      onConfirm: async () => {
        if (isIndividualArchive) {
          await archiveTransportRegulation(objId, reg.id)
        } else {
          await setRegulationCancelled(objId, reg.id, true)
        }
        setConfirm(null)
        reload()
      },
          ...
          <div style={{ marginTop: 12 }}>
            ...
            {count === 0 ? (
              <div style={{ fontSize: 13.5, color: 'var(--color-text-muted)', marginTop: 12 }}>
                Для этого транспорта нет регламентов ТО.
              </div>
            ) : (
      ...
      {regModal ? (
        <RegulationFormModal
          ...
          onSave={async (payload) => {
            if (regModal === 'new') {
              await createTransportRegulation(objId, payload)
            } else {
              await updateTransportRegulation(objId, regModal.id, payload)
            }
            setRegModal(null)
            reload()
          }}
