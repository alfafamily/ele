...
# B56-R1 (#8): тайм-ауты клиента S3 для media (botocore). Не заданы (старые
# инстансы, где переменных нет в .env) — берутся значения по умолчанию 5/60 c,
# как было в коде до вынесения в .env. connect_timeout — установка соединения
# (быстрая); read_timeout — ЛИМИТ НА ОДНУ socket-операцию, а не на весь перенос,
# поэтому большую загрузку (до Company.max_upload_mb) не обрывает. Тайм-ауты
# относятся к каналу backend↔S3 (внутри ДЦ), а не пользователь↔backend, поэтому
# от тарифа пользователя не зависят. Поднять стоит только для медленного/
# удалённого/нестабильного S3.
S3_CONNECT_TIMEOUT = env.float("S3_CONNECT_TIMEOUT", default=5)
S3_READ_TIMEOUT = env.float("S3_READ_TIMEOUT", default=60)
