...
class Place(models.Model):
    ...
    - common (МОП, место общего пользования, B45) — как рабочее место в части
      размещения имущества (Оборудование/Инструмент стационарно), но сотрудников
      за ним закреплять нельзя (кофепоинт, склад-зона и т.п.);
    ...
    # («Личный авто»). Для склада и МОП (common) не используется — за МОП
    # сотрудников закреплять нельзя. Для парковочного места взаимоисключающе
    # с transport (личные авто ЛИБО транспорт компании).
